    ...
    const confirmDelete = async () => {
        if (selectedPlace) {
            try {
                ...
                setPlaces(places.filter(place => place.id_place !== selectedPlace.id_place));
                closeDeleteModal();
            ...
            <div className={`flex-1 transition-all duration-300 ease-in-out ${sidebarOpen ? 'ml-64' : 'ml-0'}`}>
                        ...
                        <table className="min-w-full bg-white border border-gray-300 rounded-lg">
                            <thead>
                                <tr>
                                    ...
                                    <th className="py-2 px-4 border-b text-center font-semibold text-gray-700">Capacidade Máxima</th>
                            ...
                            <tbody>
                                {places.length > 0 ? (
                                    places.map((place) => (
                                        <tr key={place.id_place} className="hover:bg-gray-50 transition-colors">
                                            <td className="py-3 px-4 border-b text-center text-gray-700">{place.name}</td>
                                            <td className="py-3 px-4 border-b text-center text-gray-700">{place.max_capacity}</td>
                                            <td className="py-3 px-4 border-b text-center">
                                                <div className="flex justify-center space-x-4">
                                                    <span 
                                                        onClick={() => router.push(`/lugares/${place.id_place}`)} 
                ...
                >
                    <p className="text-gray-600">
                        Tem certeza que deseja excluir o item <strong>{selectedPlace?.name}</strong>?
                    </p>
                </CustomModal>
            </div>
        </div>
