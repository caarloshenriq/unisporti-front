    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className={`flex-1 transition-all duration-300 ease-in-out ${sidebarOpen ? 'ml-64' : 'ml-0'}`}>
        ...
        <main className="p-8 h-full overflow-auto">
          <h1 className="text-2xl font-bold mb-4">Detalhes da modalidade {mockModalityDetails.name}</h1>
          {!showClasses ? (
            <div>
              <h2>Professor Responsável: {mockModalityDetails.teacher}</h2>
              <h3>Horários Disponíveis:</h3>
              <ul>
                {mockModalityDetails.availableSchedules.map((schedule, index) => (
                  <li key={index}>{schedule}</li>
                ))}
              </ul>
              <p>{mockModalityDetails.additionalInfo}</p>
              <button onClick={toggleView} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded">
                Ver Turmas
              </button>
            </div>
          ) : (
