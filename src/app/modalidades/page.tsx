        ...
        <main className="p-8 h-full overflow-auto">
          <div className="grid gap-10 md:grid-cols-3">
            {modality.map(({ id, name, teacher }) => (
              <div key={id} className="bg-white shadow-lg rounded-lg p-8 transition-transform transform hover:scale-105 items-center hover:cursor-pointer text-center">
                <Link href={`/modalidades/${id}`}>
                  <h4 className="text-2xl font-bold mb-4 text-uniporra">{name}</h4>
                  <p className="text-uniporraBlack text-center">Professor: {teacher}</p>
                </Link>
              </div>
            ))}
          </div>
